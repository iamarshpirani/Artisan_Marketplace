    ...
    if (empty($email)) {
        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                $stmt2 = $conn->prepare("SELECT product_id, quantity FROM cart WHERE user_id = ?");
                ...
                if ($cart_result->num_rows > 0) {
                    ...
                    while ($row = $cart_result->fetch_assoc()) {
                        ...
                        // Fetch product details to add to session cart
                        $stmt3 = $conn->prepare("SELECT name, price, image_url FROM products WHERE id = ?");
                        $stmt3->bind_param("i", $product_id);
                        $stmt3->execute();
                        $product_result = $stmt3->get_result();
                        $product = $product_result->fetch_assoc();

                        // Add product details to session cart
